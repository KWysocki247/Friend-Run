...
public class PlayerControl : MonoBehaviour
{
    ...
    public GameManager GM; //Used to restart the game if player 1 died
    public PauseMenu pause; //Used to control the pause menu
    public AudioSource music; //Used to control the music
    ...
    void Update() //Script that runs once per frame to determine how the character should be updated, in terms of position and whether they are jumping.
    {
        ...
        if (Input.GetKeyDown(KeyCode.Escape)) //If a player presses the escape key
        {
            pause.gameObject.SetActive(true); //Set the game to be paused
            Time.timeScale = 0f;
        }
        if (Input.GetKeyDown(KeyCode.W)) //Determines if player presses the W key
        {
        ...
        if (Input.GetKey(KeyCode.W)) //Determines if the W key is being held down
        {
        ...
        if (Input.GetKeyUp(KeyCode.W)) //Determins when the player let's go of the W key
        {
    ...
    void OnCollisionEnter2D(Collision2D other) //Called when the player hits the death box
    {
        ...
        {
            GM.RestartPlayer1(); //Stop the game and restart it
        }
    }
    */
